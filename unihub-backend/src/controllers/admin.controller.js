...
// PUT /api/admin/utilizadores/:id/estado — bloquear ou ativar utilizador
...
// GET /api/admin/organizacoes — listar todas as organizacoes
...
// DELETE /api/admin/vagas/:id — eliminar vaga
...
// GET /api/admin/estatisticas — ver estatisticas do sistema
exports.estatisticas = async (req, res) => {
    try {
        ...
        const [[estudantes]] = await pool.query(
            'SELECT COUNT(*) AS total FROM utilizador WHERE tipo_utilizador = ?', ['ESTUDANTE']
        );
        const [[empresas]] = await pool.query(
            'SELECT COUNT(*) AS total FROM empresa'
        );
        const [[organizacoes]] = await pool.query(
            'SELECT COUNT(*) AS total FROM organizacao'
        );
